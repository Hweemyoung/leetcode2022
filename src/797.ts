...
function allPathsSourceTarget(graph: number[][]): number[][] {
    const sourceGraph: Array<number[]> = new Array<number[]>(graph.length);
...
function extendPath(curPath: number[], graph: number[][], sourceGraph: number[][]): number[][] {
    const origin = curPath[0];

    if (origin === 0) return [curPath];

    if (sourceGraph[origin] === undefined) {
        sourceGraph[origin] = [];
        const srcs = sourceGraph[origin];
        graph.forEach((directions, i) => {
            const hasOrigin: boolean = directions.indexOf(origin) !== -1;
            if (!hasOrigin) return;
            srcs.push(i);
        });
    }

    const extPaths: number[][] = [];
    const srcs = sourceGraph[origin];
